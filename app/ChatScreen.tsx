...
export default function ChatScreen() {
  ...
  useEffect(() => {
    const newHistory = messages.map((msg: any) => {
      return { role: msg.sender, parts: [{ text: msg.text }] };
    });
    setHistory(newHistory);
  }, [selectedChat, messages]);

  const saveChats = async (newChats: any) => {
  ...
  const deleteChat = (id: any) => {
    Alert.alert("Confirm Delete", "Delete this chat?", [
      { text: "Cancel" },
      {
        ...
        style: "destructive",
        onPress: () => {
          const updated = chats.filter((chat: any) => chat.id !== id);
          saveChats(updated);
          if (id === activeChatId) {
  ...
  return (
    ...
    >
      ...
      <View style={styles.chatList}>
        ...
        <FlatList
          data={chats.filter((chat: any) => !chat.archived)}
          horizontal
